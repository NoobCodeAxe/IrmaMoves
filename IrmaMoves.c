...
void parseNotationString(char* str, Move* irmaMove) {
	char directions[20] = "";
	strcpy(directions, str);
	int i = 0;
	char *p = strtok(directions, " ");
	char *array[4];
...
void save(char**board, int row, int col, char part) {
	 r = row;
	 c = col;
	 p = part;
	}
startIrma(char **board, Move *irmaMove) {
...
endIrma(char **board, Move *irmaMove) {
	restore(board);
...
irmaMath(char**board, Move*irmaMove) {
	restore(board);
	int windSpeed = irmaMove->irma.ws;
...
char **predictIrmaChange(char* str, Move *irmaMove) {
	...
	startIrma(board, irmaMove);
	printMapBoard(board);
	endIrma(board, irmaMove);
	printMapBoard(board);
	irmaMath(board, irmaMove);
